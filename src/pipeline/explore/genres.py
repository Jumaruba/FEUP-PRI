...
def explore_genres(md):
    genres_processed_path = CURRENT_PATH + "/../../data/combine/genres.csv"
    genres_df = pd.read_csv(genres_processed_path)
    stats(genres_df, md)

def explore_genres_books(md):
    genres_books_path = CURRENT_PATH + "/../../data/combine/genres_books.csv"
    genres_books_df = pd.read_csv(genres_books_path)
    genres_processed_path = CURRENT_PATH + "/../../data/combine/genres.csv"
    genres_df = pd.read_csv(genres_processed_path)
    books_path = CURRENT_PATH + "/../../data/combine/books.csv"
    ...
    plt.savefig(get_plots_path(CURRENT_PATH) + 'books_per_genre_count.jpg')
    plt.clf()
    ...
    plt.savefig(get_plots_path(CURRENT_PATH) + 'rating_per_genre.jpg')
    plt.clf()
    md.new_paragraph(md.new_inline_image(text='', path='plots/rating_per_genre.jpg'))

if __name__ == '__main__':
    md = MdUtils(file_name=CURRENT_PATH + '/../../data/explore/genres', title='Genres - Data Exploration and Characterization')
    explore_genres(md)
