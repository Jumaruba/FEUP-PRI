...
def explore_genres(md):
    genres_processed_path = get_processed_filepath("genres.csv")
    genres_df = pd.read_csv(genres_processed_path)
    stats(genres_df, md)

def explore_genres_books(md):
    genres_books_path = get_processed_filepath("genres_books.csv")
    ...
    plt.savefig(get_plots_filepath('books_per_genre_count.jpg'))
    plt.clf()
    ...
    plt.savefig(get_plots_filepath('rating_per_genre.jpg'))
    plt.clf()
