...
def explore_authors(md : MdUtils):
    ...
    md.new_header(level=3, title='Average Rating Analysis', add_table_of_contents='n')
    md.write(authors_df.drop('author_id', axis=1).describe().to_markdown(), wrap_width=0)
    md.write('\n')

    # Rating Distribution
    sns.histplot(data=authors_df, x="average_rating", binwidth=0.1)
    plt.savefig(plots_path + 'average_rating_histogram.jpg')
    plt.clf()
...
def explore_books_per_author(authors_books_df, md):
    ...
    plt.savefig(plots_path + 'books_per_author_count.jpg')
    plt.clf()
    ...
    plt.savefig(plots_path + 'books_per_author_boxplot.jpg')
    plt.clf()
    ...
    plt.savefig(plots_path + 'books_per_author_count2.jpg')
    plt.clf()
...
def explore_authors_per_book(authors_books_df, md):
    ...
    plt.savefig(plots_path + 'authors_per_book_count.jpg')
    plt.clf()
    ...
    plt.savefig(plots_path + 'authors_per_book_boxplot.jpg')
    plt.clf()
    ...
    plt.savefig(plots_path + 'authors_per_book_count2.jpg')
    plt.clf()
