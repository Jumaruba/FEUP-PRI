...
def explore_authors(md : MdUtils):
    authors_processed_path = CURRENT_PATH + "/../../data/combine/authors.csv"
    ...
    plt.savefig(get_plots_path(CURRENT_PATH) + 'average_rating_histogram.jpg')
    plt.clf()
    md.new_paragraph(md.new_inline_image(text='', path='plots/average_rating_histogram.jpg'))

def explore_authors_books(md):
    authors_books_path = CURRENT_PATH + "/../../data/combine/authors_books.csv"
...
def explore_books_per_author(authors_books_df, md):
    ...
    plt.savefig(get_plots_path(CURRENT_PATH) + 'books_per_author_count.jpg')
    plt.clf()
    ...
    plt.savefig(get_plots_path(CURRENT_PATH) + 'books_per_author_boxplot.jpg')
    plt.clf()
    ...
    plt.savefig(get_plots_path(CURRENT_PATH) + 'books_per_author_count2.jpg')
    plt.clf()
...
def explore_authors_per_book(authors_books_df, md):
    ...
    plt.savefig(get_plots_path(CURRENT_PATH) + 'authors_per_book_count.jpg')
    plt.clf()
    ...
    plt.savefig(get_plots_path(CURRENT_PATH) + 'authors_per_book_boxplot.jpg')
    plt.clf()
    ...
    plt.savefig(get_plots_path(CURRENT_PATH) + 'authors_per_book_count2.jpg')
    plt.clf()
    md.new_paragraph(md.new_inline_image(text='', path='plots/authors_per_book_count2.jpg'))

if __name__ == '__main__':
    md = MdUtils(file_name=CURRENT_PATH + '/../../data/explore/authors', title='Authors - Data Exploration and Characterization')
    explore_authors(md)
